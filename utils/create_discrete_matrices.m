...
B_c_K = zeros(n_st, 1, P.prob.T_long);
...
if P.variable_Ux_plan
    Ux_vect = P.veh.Ux * ones(P.prob.T_long, 1);
    if P.veh.Ux ~= P.veh.Ux_actual
        if P.veh.Ux <= P.veh.Ux_actual
            Ux_vect = max(P.veh.Ux_actual + P.veh.ax_min*t(2:end), P.veh.Ux)';
        else
            Ux_vect = min(P.veh.Ux_actual + P.veh.ax_max*t(2:end), P.veh.Ux)';
        end
    end
    
    A_c(1, 1, :) = -Ca_lin ./ (P.veh.mass * Ux_vect);
    A_c(1, 2, :) = Ca_lin*P.veh.b ./ (P.veh.mass * Ux_vect.^2) - 1;
    B_c_F(1, :) = 1 ./ (P.veh.mass * Ux_vect);
    d_c(1, :) = (Ca_lin.*alpha_prev + Fyr) ./ (P.veh.mass * Ux_vect);

    % linearized r dot is (a*F_y,f - b*F_y,r) / Izz
    A_c(2, 1, :) = P.veh.b*Ca_lin / P.veh.Izz;
    A_c(2, 2, :) = -P.veh.b^2*Ca_lin ./ (Ux_vect * P.veh.Izz);
    B_c_F(2, :) = P.veh.a / P.veh.Izz;
    d_c(2, :) = -(P.veh.b*Fyr + Ca_lin.*alpha_prev) / P.veh.Izz;
...
%     A_c(5, :, :) = repmat([P.veh.Ux, 0, P.veh.Ux, 0, 0], 1, 1, P.prob.T_long);
    ...
    d_c(1, :) = (Ca_lin.*alpha_prev + Fyr) / (P.veh.mass * P.veh.Ux);
...
% short term
for i=1:Tc-1
    G_short = [A_c(:,:,i), B_c_F(:, i), d_c(:,i) + B_c_K(:,:,i)*K(i);...
                 zeros(2,n_st+2)];
...
dtcorr = t(Tc+1) - t(Tc);
G_corr = [A_c(:,:,Tc), B_c_F(:, i), B_c_K(:,:,Tc), d_c(:,Tc), zeros(n_st,3);...
...
G_long = [A_c(:,:,Tc+1), B_c_F(:, i), B_c_K(:,:,Tc+1), d_c(:,Tc+1), zeros(n_st,3);...
                zeros(3,n_st+3), (1/dtlong_first)*eye(3);
                zeros(3,n_st+6)];
G_long_disc(:,:,2) = expm(G_long * dtlong_first);

for i=3:(Tl-Tc)+1
    G_long = [A_c(:,:,i+Tc-1), B_c_F(:, i), B_c_K(:,:,i+Tc-1), d_c(:,i+Tc-1), zeros(n_st,3);...
